    ...
    sendToOne: function(clients, users, data, user, type, secret_keys) {
        ...
        for(var client in clients) {
            if(clients[client].un == user) {
                console.log("encripted message " + data.message);
                const plaintext = chiper.decrypt(data.message,secret_keys[data.id.toString()].toString());
                const chipertext = chiper.encrypt(plaintext, secret_keys[clients[client].id.toString()].toString());
                data.message = chipertext;
                console.log("message sender",data.message);                
                console.log("secret sender", secret_keys[data.id.toString()]);                
                console.log("secret receiver", secret_keys[clients[client].id.toString()]);                
                if(type == 'message') {
